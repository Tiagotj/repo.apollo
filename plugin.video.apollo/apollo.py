...
url_b64_principal = 'aHR0cHM6Ly9kZXVzLWFwb2xsby5tbC9BUE9MTE8xL0JBU0UtUFJJTkNJUEFML0Jhc2UtUHJpbmNpcGFsLmh0bWw='
...
####  SUPORTE filme4k  #################################################################
title_menu2 = '-> INSTALE PLUGIN PARA FILMES 4K'
#######################################################################################
#######################################################################################
###  DESCRIÇÃO DO ADDON ###############################################################
...
#### MENU filme4k ################################################################
titulo_filme4k = 'filme4k'
thumbnail_filme4k = 'https://i.imgur.com/67UUio3.png'
fanart_filme4k = 'https://i.imgur.com/Y4QXRab.jpg'
#### DESCRIÇÃO ADVANCED filme4k ###########################################################
url_b64_filme4k_descricao = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L0JVYk5CWlFB'
url_filme4k_descricao = base64.b64decode(url_b64_filme4k_descricao)
#### DIALOGO ADVANCED filme4k - SERVIDOR DESATIVADO - CLICK ###################################
url_b64_filme4k_dialogo = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L0JVYk5CWlFB'
url_filme4k_dialogo = base64.b64decode(url_b64_filme4k_dialogo)
...
url_b64_bem_vindo = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L3F0NmhqV0VY'
thumbnail_boas_vindas = 'https://i.imgur.com/fHRcl7O.png'
fanart_boas_vindas = 'https://i.imgur.com/Y4QXRab.jpg'
...
#Modulos
#exec(module_import('https://pastebin.com/raw/Lb4cHSRQ'))

def SUPORTE(msg):
   suporte = addon.getSetting('suporte')
   if suporte == 'true':
    addDir(title_menu2,url_branco,44,thumbnail_filme4k,FANART,descricao(url_title_descricao2),'','','','','','','','','','','','','','','')
...
def init_SUPORTE(msg):
    ...
    if link == 5:
     ...
     #xbmcgui.Dialog().ok('[B][COLOR deepskyblue]AVISO IMPORTANTE![/COLOR][/B]','[COLOR orange][B]PARA QUE AS CONFIGURAÇÕES DO (filme4k) SEJA ATIVADO NO MENU INICIAL DO ADD-ON SAIA E ENTRE NOVAMENTE NO ADD-ON LOGO APÓS TER SAIDO VOCÊ JÁ PODÊ ACESSAR O filme4k!! OBS. VOCÊ TEM QUE TER CONTRATADO ANTES![/COLOR][/B]')
     xbmc.executebuiltin("XBMC.Container.Refresh(%s?mode=None, replace)"%sys.argv[0])
     xbmcplugin.endOfDirectory(int(sys.argv[1]))
            
def filme4k():
    username_filme4k = addon.getSetting('username')
    password_filme4k = addon.getSetting('password')
    tipo_servidor = addon.getSetting('servidor')
    filme4k_menu = addon.getSetting('exibirfilme4k')
    saida_transmissao = addon.getSetting('saida')
    if filme4k_menu == 'true':
        if tipo_servidor=='filme4k':
            url = 'http://sv1.iptvcasa.online:25461/get.php?username=%s&password=%s&type=m3u_plus&output=%s'%(username_filme4k,password_filme4k,saida_transmissao)
            addDir(titulo_filme4k,url,1,thumbnail_filme4k,fanart_filme4k,descricao(url_filme4k_descricao),'','','','','','','','','','','','','','','')
        if tipo_servidor=='Nenhum':
            addDir(titulo_filme4k,'',42,thumbnail_filme4k,fanart_filme4k,descricao(url_filme4k_descricao),'','','','','','','','','','','','','','','')
    username_filme4k = addon.getSetting('username2')
    password_filme4k = addon.getSetting('password2')
    tipo_servidor = addon.getSetting('servidor2')
    filme4k_menu = addon.getSetting('exibirfilme4k2')
    saida_transmissao = addon.getSetting('saida2')
    if filme4k_menu == 'true':
        if tipo_servidor=='filme4k':
            url = 'http://cloudsteam.club:8080/get.php?username=%s&password=%s&type=m3u_plus&output=%s'%(username_filme4k,password_filme4k,saida_transmissao)
            addDir(titulo_filme4k2,url,1,thumbnail_filme4k2,fanart_filme4k,descricao(url_filme4k_descricao),'','','','','','','','','','','','','','','')
        if tipo_servidor=='Nenhum':
            addDir(titulo_filme4k2,'',42,thumbnail_filme4k2,fanart_filme4k2,descricao(url_filme4k_descricao),'','','','','','','','','','','','','','','')
...
def CheckUpdate(msg): #54
	try:
		...
		if uversao != Versao:
			...
			xbmcgui.Dialog().ok('[COLOR orange][B]APOLLO[/COLOR][/B]', "[COLOR deepskyblue][B] O ADD-ON JÁ ESTÁ NA ÚLTIMA VERSÃO![/COLOR][/B][COLOR orange][B] 1.4.9[/COLOR][/B] "+Versao+"[COLOR deepskyblue][B] ATUALIZAÇÕES SÃO AUTOMÁTICAS SE NÃO ATUALIZA ENTRE NO SITE[/COLOR][/B][COLOR orange][B] http://deus-apollo.ml[/COLOR][/B]")
			#xbmc.executebuiltin("XBMC.Container.Refresh()")
	except urllib2.URLError, e:
		if msg==True:
			xbmcgui.Dialog().ok('Apollo', "Não foi possível checar")
...
def getSoup(url,data=None):
        ...
        if url.startswith('http://') or url.startswith('https://'):
            ...
            if '$$LSProEncKey=' in url:
                ...
                url=url.replace(rp,"")
            try:  #note que foi ADC 
                data = base64.b32decode(makeRequest(url)[::-1]).decode('utf-8')  #note que foi ADC 
            except:
...
#                            urllib2.getproxies= lambda: {}
                        ...
                        req.add_header('User-Agent', 'Mozilla/5.0 (Windows NT 6.1; rv:14.0) Gecko/20100101 Firefox/14.0.1')#useragent
                        proxytouse=None
...
#                print 'setting as folder in link'
        ...
        #if isFolder:
        if allinfo==None or len(allinfo) <1:
            liz.setInfo(type="Video", infoLabels={ "Title": name, "Plot": description, "Genre": genre, "dateadded": date, "Year": year, "Director": director, "Duration": duration, "Premiered": premiered, "Studio": studio, "Mpaa": rate, "Originaltitle": originaltitle, "Country": country, "Rating": rating, "Userrating": userrating, "Votes": votes, "Aired": aired })
        else:
            liz.setInfo(type="Video", infoLabels=allinfo)
        liz.setProperty("Fanart_Image", fanart)
...
if mode==None:
    ...
    addon_log("infofilme4k")    
    ##xbmcaddon.Addon().openSettings()
    #xbmcgui.Dialog().ok('[COLOR gold][B](filme4k) [/B][/COLOR][COLOR aquamarine][B]AREA ASSINANTES[/B][/COLOR]', dialog4)
    xbmcgui.Dialog().ok(titulo_filme4k, descricao(url_filme4k_dialogo))
    ##xbmc.executebuiltin("XBMC.Container.Refresh")
